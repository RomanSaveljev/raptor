...
# the Raptor class and calls its methods to perform a build based
# on command-line parameters, or by explicitly creating a Raptor
...
import raptor.timing as timing
from raptor.makefile import MakefileSet
from raptor.sysdef import SystemModel
from raptor.filter_list import FilterList
from raptor.pluginbox import PluginBox
from raptor.buildrecord import BuildRecord
...
defaults = {
		...
		"systemPlugins" : generic_path.Path("raptor/plugins"),
...
class ModelNode(object):
	...
	def unique_filename(self): 
		...
		return raptor.utilities.sanitise(hash(id))
...
class Layer(ModelNode):
	...
	def __init__(self, name, componentlist=[]):
		""" componentlist may be a list of items of type Component xor type sysdef.SystemModelComponent
	...
	def unique_filename(self): 
		...
		return raptor.utilities.sanitise(self.name)


	@classmethod
	def from_system_model(cls, name, sysmodel_componentlist):
		""" A factory method to build a layer from a sysdef.SystemModelComponent
	...
	def unfurl(self, build):
		...
		if len(components) > 0:
			try:
				...
				metaReader = raptor.meta.MetaReader(build, build.buildUnitsToBuild)
			...
			except raptor.meta.MetaDataError as e:
				build.Error(e.Text)
	...
	def meta_realise(self, build):
		...
		binding_makefiles = MakefileSet(str(tm.Dir()), build.maker.selectors, makefiles=None, filenamebase=str(tm.File()))
		...
		for block in component_blocks:
			loop_number += 1
			specNode = raptor.data.Specification("metadata_" + self.name)
			...
			try:
				sys_def_writer = SystemModel(build, aDoRead=False)
			...
			var.AddOperation(raptor.data.Set("PP_SYSTEM_DEFINITION", pp_system_definition))
			var.AddOperation(raptor.data.Set("MAKEFILE_PATH", makefile_path))
			var.AddOperation(raptor.data.Set("CONFIGS", configList))
			var.AddOperation(raptor.data.Set("CLI_OPTIONS", cli_options))
			...
			if not build.doExport:
				doexport_str = ''
			var.AddOperation(raptor.data.Set("DOEXPORT", doexport_str ))

			# Pass on '-n' (if specified) to the makefile-generating sbs calls
			if build.noBuild:
				var.AddOperation(raptor.data.Set("NO_BUILD", "1"))
...
class Raptor(object):
	...
	def __init__(self, home = None, commandline = [], dotargets = True, logger = None, load_defaults = True):
		...
		if len(commandline) > 0:
			...
			# assuming self.CLI = "cli"
			if not raptor.cli.GetArgs(self, self.args):
	...
	def _default_setup(self, home = None):
		...
		self.cache = raptor.cache.Cache(self)
		...
		self.filesystem = raptor.utilities.getOSFileSystem()
		...
		if self.load_defaults:
			...
			if self.raptorXML.isFile():
				...
				try:
					...
					evaluator = self.GetEvaluator( None, raptor.data.BuildUnit(var.name,[var]) )
	...
	def _load_cache(self):
		...
		self.InfoStartTime(object_type = "config_cache", task = "load", key="cache")
		...
		self.metaeval    = self.GetEvaluator(None, raptor.data.BuildUnit(self.metavariant.name, [self.metavariant]) )
		self.InfoEndTime(object_type = "config_cache", task = "load", key="cache")
	...
	def AddSourceTarget(self, filename):
		...
		sourceTarget = 'SOURCETARGET_' + raptor.utilities.sanitise(str(sourceTarget))
	...
	def PrintVersion(self,dummy):
		global name
		print("{0} version {1}".format(name, raptor.version.fullversion()))
	...
	def Introduction(self):
		...
		self.Info("{0}: version {1}".format(name, raptor.version.fullversion()))
	...
	def GetBuildUnitsToBuild(self, configNames):
		...
		GetBuildUnitsToBuild(["armv5", "winscw"])
		...
		buildUnitsToBuild = raptor.data.GetBuildUnits(configNames, self.cache, self)
	...
	def CheckToolset(self, evaluator, configname):
		...
		to the toolset for this build (e.g. the make class)."""
		if self.toolset is None:
			if self.toolcheck == 'on':
				self.toolset = raptor.data.ToolSet(log=self)
			elif self.toolcheck == 'forced' :
				self.toolset = raptor.data.ToolSet(log=self, forced=True)
			else:
				return True
		...
		for c in configsToBuild:
			...
			if iface:
				if iface in configWide:
					...
					# create a new node
					filter = raptor.data.Filter(name = "config_wide")
					filter.AddConfigCondition(c.name)
					for i in iface.split():
						spec = raptor.data.Specification(i)
						spec.SetInterface(i)
	...
	def GetEvaluator(self, specification, configuration, gathertools=False):
		...
		return raptor.data.Evaluator(specification, configuration, gathertools=gathertools, cache = self.cache)
	...
	def InitPlugins(self):
		...
		self.pbox = PluginBox(str(self.systemPlugins))

		# Try to load up plugins from the era before raptor-as-a-package
		oldplugins = self.home.Append("python/plugins")
	...
	def OpenLog(self):
		...
		try:
			...
			self.out.write("<buildlog sbs_version=\"{0}\" xmlns=\"{1}\" xmlns:progress=\"{2}\""
				       ...
				       raptor.version.fullversion(), namespace, progress_namespace, namespace, schema))
			self.logOpen = True
		except Exception as e:
	...
	def InfoDiscovery(self, object_type, count):
		if self.timing:
			try:
				self.out.write(timing.Timing.discovery_string(object_type = object_type,
						count = count))
	...
	def InfoEndTime(self, object_type, task, key):
		if self.timing:
			try:
				self.out.write(timing.Timing.end_string(object_type = object_type,
						task = task, key = key))
			except Exception as exception:
	...
	def GetLayersFromCLI(self):
		...
		if self.systemDefinitionFile != None:
			systemModel = SystemModel(self, self.systemDefinitionFile, self.systemDefinitionBase)
	...
	def Query(self):
		...
		import raptor.api
		context = raptor.api.Context(self)
		
		# intialise the plugs in case they are needed for the query
		self.InitPlugins()
		
		print("<sbs version='{0}'>".format(raptor.version.numericversion()))
		
		for q in self.queries:
			try:
				print(context.stringquery(q))
	...
	def Build(self):
		...
		try:
			...
			if not self.maker:
				try:
					self.maker = raptor.make.MakeEngine(self, self.makeEngine)
				except raptor.make.BadMakeEngineException as e:

...
class FilterTiming(filter_interface.Filter):
	...
	def open(self, raptor_instance):
		"""
			Open a log file with the same name as the Raptor log file, with
					...
					raptor.timing tags from the Raptor output
			Parameters:
	...
	def write(self, text):
		...
		if "<progress:discovery " in text:
			...
			attributes = raptor.timing.Timing.extract_values(source = text)
			...
			self.out.write(raptor.timing.Timing.custom_string(tag = "duration",
	...
	def summary(self):
		"""
			Print out extra raptor.timing info
		"""
		total_time = 0.0

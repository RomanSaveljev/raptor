...
if not HostPlatform.IsKnown(HostPlatform.hostplatform):
	raise Exception("raptor.data module loaded on an unrecognised platform '{0}'. Expected one of {1}".format(HostPlatform.hostplatform, str(HostPlatform.hostplatforms)))


# raptor.data module classes
...
class Group(Model, Config):
	...
	def Valid(self):
		return str(self.name) != str("") and len(self.childRefs)>0
...
class Tool(object):
	...
	def check(self, shell, evaluator, log = raptor.utilities.nulllog):
		...
		if self.name.upper() == "SBS":
			versionoutput = raptor.version.fullversion()
		else:
			try:
...
class ToolSet(object):
	...
	def __init__(self, log = raptor.utilities.nulllog, forced = False):
...
class XMLConfigParser(xml.sax.handler.ContentHandler, xml.sax.handler.ErrorHandler):
	"A SAX parser that can construct raptor.data objects"
	
	# define these methods in your subclass

	def __init__(self, log):
		""" log  - an object with Info, Warn, Error methods"""
		self.estack = deque()
		self.build = []
		self.log = log
		self.ok = True
	...
	def startElement(self, name, attributes):
		...
		# This subtree is turned off e.g for this platform
		if self.ignorelevels > 0:
			self.ignorelevels += 1
			return
		elif name == "build":
			if 'xsi:schemaLocation' in attributes:
				sl = attributes['xsi:schemaLocation']
				if sl.find(xsdIgnore) != -1:
					raise InvalidConfigFile("invalid XML schema detected - found {0} require {1}".format(sl,xsdVersion))
				if sl.find(xsdVersion) == -1:
					raise InvalidConfigFile("old or invalid schema - found {0}, looking for {1}".format(sl, xsdVersion))
			self.estack.append(("build", self.build))
			return


		# Check if this item fits this platform (<build> doesn't count)
		if 'host' in attributes and not HostPlatform.IsHost(attributes['host']):
			# we still have to parse the  subtree but we're not creating a data model
			# objects for any of it.
			self.ignorelevels += 1
		else:
			...
			try:
				...
				self.log.Warn("Unknown element {0}".format(name))
				return
			...
			for k,v in attributes.items():
				try:
					...
					self.log.Warn("Cannot set attribute {0} for element {1}".format(k,v))

			if self.estack[-1][0] == 'build':
				self.estack[-1][1].append(model)
			else:
				if self.estack[-1][1]:
					self.estack[-1][1].AddChild(model)
		
			self.estack.append((name,model))
	...
	def endElement(self, name):
		...
		try:
			node = self.estack.pop()
			if node[0] != 'build':
				if not node[1].Valid():
					raise InvalidChildError("Model {0} not valid".format(node[1].name))
			
		except IndexError as e:
			raise XMLError("XML Document not terminated by a </build> tag")
	...
	def error(self, exception):
		...
		self.log.Warn("configuration parse error: '{0}'".format(str(exception)))
		pass
		
	def fatalError(self, exception):
		"""the parser thinks an error occurred which should stop everything"""
		self.ok = False
		raise XMLError("'{0}'".format(str(exception)))
		
	def warning(self, exception):
		"""the parser found something to complain about that might not matter"""
		self.log.Warn("configuration parse: '{0}'".format(str(exception)))
		pass
	...
	def read_file(self, filename):
		...
		try:
			...
			# check all the configuration items we loaded up:
		except InvalidConfigFile as e:
			self.log.Info("Ignoring invalid config file: {0}".format(filename))

		return self.build # all the constructed data model items

	@staticmethod
	def read(log, filename):
		c = XMLConfigParser(log = log)
		return c.read_file(filename)

# end of the raptor.data module

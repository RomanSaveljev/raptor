...
import raptor.generic_path as generic_path
...
def getPreProcessorCommentDetail (aPreProcessorComment):
	...
	commentDetail = []
	commentMatch = re.search(r'# (?P<LINENUMBER>\d+) "(?P<FILENAME>.*)"', aPreProcessorComment)
...
class PreProcessor(raptor.utilities.ExternalTool):
	...
	def __init__(self, aPreProcessor,
				 ...
				 aPreIncludeOption,
				 aRaptor):
		raptor.utilities.ExternalTool.__init__(self, aPreProcessor)
	...
	def call(self, aArgs, sourcefilename):
		...
		try:
			...
			# This deals with python3/2 differences on linux without
			# needing to specify version numbers or operating systems:
			if type(self._ExternalTool__Output) is not str: 
				self._ExternalTool__Output = self._ExternalTool__Output.decode()
...
class MetaDataFile(object):
	...
	def getContent(self, aBuildPlatform):
		...
		if adepfilename:
			...
			if raptor.utilities.getOSPlatform().startswith("win"):
...
class Export(object):
	...
	def __init__(self, aBldInfFile, aExportsLine, aType):
		...
		sourcepath = generic_path.Path(raptor.utilities.resolveSymbianPath(str(aBldInfFile), source_spec))
		...
		if dest_spec:
			...
			dest_list = raptor.utilities.resolveSymbianPath(str(aBldInfFile), dest_spec, aType, aSubType)
			...
			if isinstance(dest_list, list):
				...
				dest_list = [process_dest(d) for d in dest_list]
			else:
		...
		try:
			...
			if metaFile:
				for line in metaFile.readlines():
					...
					if defaultOptionMatch and defaultOptionMatch.group('VARIABLE').upper() not in self.__Options:
...
class BldInfFile(MetaDataFile):
	...
	def outputPathFragment(bldinfpath):
		...
		uniqueid.update(absroot_str.encode("utf-8"))
...
class MMPRaptorBackend(MMPBackend):
	...
	def __init__(self, aRaptor, aMmpfilename, aBldInfFilename):
		...
		self.BuildVariant = raptor.data.Variant(name = "mmp")
		
		if self.__Raptor.no_metadata_depend:
			self.__debug("No metadata depend requested. Setting PROJECT_META_DEP to blank.")
			self.BuildVariant.AddOperation(raptor.data.Set("PROJECT_META_DEP", ""))
		else:
			self.__debug("Setting PROJECT_META_DEP to \"{0}\".".format(str(aMmpfilename)))
			self.BuildVariant.AddOperation(raptor.data.Set("PROJECT_META_DEP", str(aMmpfilename)))
		
		self.BuildVariant.AddOperation(raptor.data.Set("PROJECT_META", str(aMmpfilename)))
		self.ApplyVariants = []
		...
		self.__sourcepath = raptor.utilities.resolveSymbianPath(self.__currentMmpFile, "")
	...
	def doSetSwitch(self,s,loc,toks):
		...
		if varname=='ARMINC':
			...
			self.BuildVariant.AddOperation(raptor.data.Set(prefix+varname, "1"))
			...
			self.BuildVariant.AddOperation(raptor.data.Set("LTCG",""))
			self.__debug( "Set switch " + varname + " OFF")
			
		elif varname == 'NOMULTIFILECOMPILATION':
			self.BuildVariant.AddOperation(raptor.data.Set("MULTIFILE_ENABLED",""))
			self.__debug( "Set switch " + varname + " OFF")
			...
			self.BuildVariant.AddOperation(raptor.data.Set(varname,"1"))
	...
	def doAssignment(self,s,loc,toks):
		...
		if varname=='TARGET':
			...
			self.BuildVariant.AddOperation(raptor.data.Set("REQUESTEDTARGETEXT", self.__TARGETEXT.lower()))
			...
			self.BuildVariant.AddOperation(raptor.data.Set("TARGET", self.__TARGET))
			# case folding: case insensitive for resources
			if self.__Raptor.doCaseFolding_rsg:
				self.BuildVariant.AddOperation(raptor.data.Set("TARGET_var", lowercase_TARGET))
			else:
				self.BuildVariant.AddOperation(raptor.data.Set("TARGET_var", self.__TARGET))
			...
			if  self.__targettype.lower() == "none":
				self.BuildVariant.AddOperation(raptor.data.Set("TARGET", ""))
				self.BuildVariant.AddOperation(raptor.data.Set("TARGET_var",""))
				self.BuildVariant.AddOperation(raptor.data.Set("REQUESTEDTARGETEXT", ""))
			self.BuildVariant.AddOperation(raptor.data.Set(varname,toks[1].lower()))

		elif varname=='TARGETPATH':
			value = toks[1].lower().replace('\\','/')
			self.__debug("Set "+varname+" to " + value)
			self.BuildVariant.AddOperation(raptor.data.Set(varname, value))

		elif varname=='OPTION' or varname=='LINKEROPTION':
			self.__debug("Set "+toks[1]+varname+" to " + str(toks[2]))
			self.BuildVariant.AddOperation(raptor.data.Append(varname+"_"+toks[1].upper()," ".join(toks[2])))
			...
			if "armasm" in toks[1].lower():
				...
				for searchReplacePair in searchReplacePairs:
					...
					self.BuildVariant.AddOperation(raptor.data.Append(varname+"_"+toks[1].upper(),searchReplacePair))
			...
			for path in toks[1]:
				...
				resolved = raptor.utilities.resolveSymbianPath(self.__currentMmpFile, path).strip().rstrip('\/')
			...
			self.BuildVariant.AddOperation(raptor.data.Set(varname,"".join(libName)))
			...
			self.BuildVariant.AddOperation(raptor.data.Set(varname, toks[1]))
			...
			self.BuildVariant.AddOperation(raptor.data.Set(varname, hexoutput))
		elif varname=='VERSION':
			if toks[-1] == "EXPLICIT":
				self.__explicitversion = True
				self.BuildVariant.AddOperation(raptor.data.Set("EXPLICITVERSION", "1"))
			...
			if vm is not None:
				...
				self.BuildVariant.AddOperation(raptor.data.Set(varname, "{0}.{1}".format(major, minor)))
				self.BuildVariant.AddOperation(raptor.data.Set("VERSIONHEX", self.__versionhex))
			...
			self.BuildVariant.AddOperation(raptor.data.Set(varname+"MIN", "%x" % min))
			self.__debug("Set "+varname+"MIN  OPTION to '%x' (hex)" % min )
			self.BuildVariant.AddOperation(raptor.data.Set(varname+"MAX", "%x" % max))
			...
			self.BuildVariant.AddOperation(raptor.data.Set(varname+"MIN_DEC_KB", "%d" % min_dec_kb))
			self.__debug("Set "+varname+"MIN  OPTION KB to '%d' (dec)" % min_dec_kb )
			self.BuildVariant.AddOperation(raptor.data.Set(varname+"MAX_DEC_KB", "%d" % max_dec_kb))
			...
			if toks[1].lower().startswith('0x'):
				...
				stack = int(toks[1],10)
			self.BuildVariant.AddOperation(raptor.data.Set(varname, "%x" % stack))
			...
			if prio not in MMPRaptorBackend.epoc32priorities:
				self.__Raptor.Error("Priority setting '{0}' is not a valid priority - should be one of {1}.".format(prio, list(MMPRaptorBackend.epoc32priorities.values())))
			else:
				self.__debug("Set "+toks[0]+" to " +  MMPRaptorBackend.epoc32priorities[prio])
				self.BuildVariant.AddOperation(raptor.data.Set(varname,MMPRaptorBackend.epoc32priorities[prio]))
		elif varname=='ROMTARGET' or varname=='RAMTARGET':
			if len(toks) == 1:
				self.__debug("Set "+toks[0]+" to <none>" )
				self.BuildVariant.AddOperation(raptor.data.Set(varname,"<none>"))
			else:
				toks1 = str(toks[1]).replace("\\","/")
				if toks1.find(","):
					toks1 = re.sub("[,'\[\]]", "", toks1).replace("//","/")
				self.__debug("Set "+toks[0]+" to " + toks1)
				self.BuildVariant.AddOperation(raptor.data.Set(varname,toks1))
			...
			if not str(toks[1]).lower() in self.armfpu_options:
				...
				self.__debug("Set "+toks[0]+" to " + str(toks[1]))
				self.BuildVariant.AddOperation(raptor.data.Set(varname,str(toks[1])))
		else:
			self.__debug("Set "+toks[0]+" to " + str(toks[1]))
			self.BuildVariant.AddOperation(raptor.data.Set(varname,"".join(toks[1])))
	...
	def doAppend(self,s,loc,toks):
		...
		for item in toks[1]:
			...
			if name=='LIBRARY' or name=='DEBUGLIBRARY':
				...
				item = raptor.utilities.resolveSymbianPath(self.__bldInfFilename, item)
				
			self.BuildVariant.AddOperation(raptor.data.Append(name,item," "))
			
			# maintain a debug library list, the same as LIBRARY but with DEBUGLIBRARY values
			# appended as they are encountered
			if name=='LIBRARY' or name=='DEBUGLIBRARY':
				self.BuildVariant.AddOperation(raptor.data.Append("LIBRARY_DEBUG",item," "))			
	...
	def doUIDAssignment(self,s,loc,toks):
		...
		self.BuildVariant.AddOperation(raptor.data.Set("UID2", hexoutput))

		if len(toks[1]) > 1:
			hexoutput = MMPRaptorBackend.canonicalUID(toks[1][1])
			self.__debug( "Set UID3 to {0}".format(hexoutput))
			self.BuildVariant.AddOperation(raptor.data.Set("UID3", hexoutput))
	...
	def doSourcePathAssignment(self,s,loc,toks):
		self.__currentLineNumber += 1
		self.__sourcepath = raptor.utilities.resolveSymbianPath(self.__currentMmpFile, toks[1])
	...
	def doOldResourceAssignment(self,s,loc,toks):
		...
		for rss in toks[1]:
			variant = raptor.data.Variant()

			source = generic_path.Join(self.__sourcepath, rss)
			variant.AddOperation(raptor.data.Set("SOURCE", str(source)))
			self.__resourceFiles.append(str(source))

			target = source.File().rsplit(".", 1)[0]	# remove the extension
			variant.AddOperation(raptor.data.Set("TARGET", target))
			
			if self.__Raptor.doCaseFolding_rsg:
				variant.AddOperation(raptor.data.Set("TARGET_var", target.lower()))
				header = target.lower() + ".rsg"
			else:
				variant.AddOperation(raptor.data.Set("TARGET_var", target))
				header = target + ".rsg"
			 
			variant.AddOperation(raptor.data.Set("HEADER", header))

			if sysRes:
				dsrtp = self.getDefaultSystemResourceTargetPath()
				variant.AddOperation(raptor.data.Set("TARGETPATH", dsrtp))
	...
	def doStartResource(self,s,loc,toks):
		...
		self.__currentResourceVariant = raptor.data.Variant()
		self.__currentResourceVariant.AddOperation(raptor.data.Set("SOURCE", self.__current_resource))
		...
		self.__currentResourceVariant.AddOperation(raptor.data.Set("TARGET", target))

		if self.__Raptor.doCaseFolding_rsg:
			self.__currentResourceVariant.AddOperation(raptor.data.Set("TARGET_var", target.lower()))
			self.__current_resource_header = target.lower() + ".rsg"
		else:
			self.__currentResourceVariant.AddOperation(raptor.data.Set("TARGET_var", target))
	...
	def doResourceAssignment(self,s,loc,toks):
		...
		if varname == "TARGET":
			...
			self.__currentResourceVariant.AddOperation(raptor.data.Set(varname,target))
		if varname == "TARGETPATH":
			varvalue=varvalue.replace('\\','/')
			self.__debug("Set resource "+varname+" to " + varvalue)
			self.__currentResourceVariant.AddOperation(raptor.data.Set(varname,varvalue))
		else:
			self.__debug("Set resource "+varname+" to " + varvalue)
			self.__currentResourceVariant.AddOperation(raptor.data.Set(varname,varvalue))
		return "OK"
	...
	def doResourceAppend(self,s,loc,toks):
		...
		for item in toks[1]:
			if varname == "LANGUAGES":
				item = item.lower()
			self.__currentResourceVariant.AddOperation(raptor.data.Append(varname,item))
		return "OK"
	...
	def doResourceSetSwitch(self,s,loc,toks):
		...
		if name == "HEADER":
			...
			self.__currentResourceVariant.AddOperation(raptor.data.Set(name, value))
	...
	def doEndResource(self,s,loc,toks):
		...
		if self.__headerspecified:
			...
			self.__currentResourceVariant.AddOperation(raptor.data.Set("HEADER", self.__current_resource_header))

		if self.__headeronlyspecified:
			self.__debug("Set resource switch HEADERONLY " + self.__current_resource_header)
			self.__currentResourceVariant.AddOperation(raptor.data.Set("HEADER", self.__current_resource_header))
			self.__currentResourceVariant.AddOperation(raptor.data.Set("HEADERONLY", "True"))
	...
	def doStartBitmap(self,s,loc,toks):
		...
		self.__currentBitmapVariant = raptor.data.Variant(name = toks[1].replace('.','_'))
		...
		self.__currentBitmapVariant.AddOperation(raptor.data.Set("BMTARGET", toks[1]))
		self.__currentBitmapVariant.AddOperation(raptor.data.Set("BMTARGET_lower", toks[1].lower()))
		self.__currentBitmapVariant.AddOperation(raptor.data.Set("SOURCE", ""))
		return "OK"
	...
	def doBitmapAssignment(self,s,loc,toks):
		...
		self.__currentBitmapVariant.AddOperation(raptor.data.Set(name,value))
		return "OK"

	def doBitmapSourcePathAssignment(self,s,loc,toks):
		self.__currentLineNumber += 1
		self.__debug("Previous bitmap sourcepath:" + self.__bitmapSourcepath)
		self.__bitmapSourcepath = raptor.utilities.resolveSymbianPath(self.__currentMmpFile, toks[1])
	...
	def doBitmapSourceAssignment(self,s,loc,toks):
		...
		self.__currentBitmapVariant.AddOperation(raptor.data.Append("SOURCE", sources))
		return "OK"

	def doBitmapSetSwitch(self,s,loc,toks):
		self.__currentLineNumber += 1
		self.__debug( "Set bitmap switch "+toks[0]+" ON")
		self.__currentBitmapVariant.AddOperation(raptor.data.Set(toks[0].upper(), "1"))
		return "OK"
	...
	def doStartStringTable(self,s,loc,toks):
		...
		self.__currentStringTableVariant = raptor.data.Variant(name = uniqname)
		self.__currentStringTableVariant.AddOperation(raptor.data.Set("SOURCE", source))
		self.__currentStringTableVariant.AddOperation(raptor.data.Set("EXPORTPATH", ""))
		...
		self.__currentStringTableVariant.AddOperation(raptor.data.Set("TARGET", self.__stringtable))
	...
	def doStringTableAssignment(self,s,loc,toks):
		...
		if varname == "EXPORTPATH":
			finalvalue = raptor.utilities.resolveSymbianPath(self.__currentMmpFile, varvalue)
			self.__stringtableExported = True
		else:
			finalvalue = varvalue

		self.__debug("Set stringtable "+varname+" to " + finalvalue)
		self.__currentStringTableVariant.AddOperation(raptor.data.Set(varname,finalvalue))
		return "OK"
	...
	def doStringTableSetSwitch(self,s,loc,toks):
		...
		if toks[0].upper()== "HEADERONLY":
			...
			self.__currentStringTableVariant.AddOperation(raptor.data.Set(toks[0].upper(), "1"))
		return "OK"
	...
	def finaliseIncludes(self):
		...
		if self.__traces_sentinel in self.__userinclude_list:
			...
			self.__userinclude_list[index] = raptor.utilities.resolveSymbianPath(self.__currentMmpFile, path).strip().rstrip('\/')
		...
		self.BuildVariant.AddOperation(raptor.data.Append('USERINCLUDE', userincludes))
		
		systemincludes = " ".join(self.__systeminclude_list)
		self.__debug("System includes are: {0}".format(systemincludes))
		self.BuildVariant.AddOperation(raptor.data.Append('SYSTEMINCLUDE', systemincludes))
	...
	def finalise(self, aBuildPlatform):
		...
		if self.__TARGET:
			...
			self.BuildVariant.AddOperation(raptor.data.Set("RESOLVED_DEFFILE", resolvedDefFile))
			...
			self.BuildVariant.AddOperation(raptor.data.Set("RESOLVED_DEFFILE_SECONDARY", secondaryDefFile))

		# If a deffile is specified, an FLM will put in a dependency.
		# If a deffile is specified then raptor.meta will guess a name but:
		...
		self.BuildVariant.AddOperation(raptor.data.Set("DEFFILE", resolvedDefFile))
		self.__debug("Set DEFFILE to " + resolvedDefFile)
		self.BuildVariant.AddOperation(raptor.data.Set("DEFFILEKEYWORD", deffile_keyword))
		...
		if tp:
			for i,var in enumerate(self.ResourceVariants):
				...
				needTP = True
				for op in var.ops:
					if isinstance(op, raptor.data.Set) \
					and op.name == "TARGETPATH":
						needTP = False
						break
				if needTP:
					self.ResourceVariants[i].AddOperation(raptor.data.Set("TARGETPATH", tp))
		...
		for resourceFile in self.__resourceFiles:
			self.BuildVariant.AddOperation(raptor.data.Append("RESOURCEFILES", resourceFile))

		for i,var in enumerate(self.ResourceVariants):
			self.ResourceVariants[i].AddOperation(raptor.data.Set("MAIN_TARGET_lower", self.__TARGET.lower()))
			self.ResourceVariants[i].AddOperation(raptor.data.Set("MAIN_REQUESTEDTARGETEXT", self.__TARGETEXT.lower()))

		# Create Capability variable in one SET operation (more efficient than multiple appends)
		
		self.BuildVariant.AddOperation(raptor.data.Set("CAPABILITY","".join(self.capabilities)))
		...
		self.BuildVariant.AddOperation(raptor.data.Set("CAPABILITYFLAG1", capabilityFlag1))
		self.__debug ("Set CAPABILITYFLAG1 to " + capabilityFlag1)
		self.BuildVariant.AddOperation(raptor.data.Set("CAPABILITYFLAG2", capabilityFlag2))
		...
		if "codepaging" in self.__resolved_keywords:
			if self.__resolved_keywords["codepaging"] == "PAGEDCODE":
				...
				self.BuildVariant.AddOperation(raptor.data.Set("PAGED", "1"))
				self.__debug("Set value PAGEDCODE_OPTION = paged")
				self.BuildVariant.AddOperation(raptor.data.Set("PAGEDCODE_OPTION", "paged"))
				explicitpagedcode = True
			else:
				self.__debug("Set switch PAGED OFF")
				self.BuildVariant.AddOperation(raptor.data.Set("PAGED", "0"))
				self.__debug("Set value PAGEDCODE_OPTION = unpaged")
				self.BuildVariant.AddOperation(raptor.data.Set("PAGEDCODE_OPTION", "unpaged"))
		...
		if "datapaging" in self.__resolved_keywords:
			if self.__resolved_keywords["datapaging"] == "PAGEDDATA":
				...
				self.BuildVariant.AddOperation(raptor.data.Set("PAGEDDATA_OPTION", "paged"))
			else:
				self.__debug("Set value PAGEDDATA_OPTION = unpaged")
				self.BuildVariant.AddOperation(raptor.data.Set("PAGEDDATA_OPTION", "unpaged"))
		...
		if compressionkeyword:
			...
			self.BuildVariant.AddOperation(raptor.data.Set(compressionkeyword, "1"))
		
		# Set Debuggable
		self.BuildVariant.AddOperation(raptor.data.Set("DEBUGGABLE", self.__debuggable))

		if self.__explicitversion:
			self.BuildVariant.AddOperation(raptor.data.Append("UNIQUETARGETPATH","$(TARGET_var)_$(VERSIONHEX)_$(REQUESTEDTARGETEXT)",'/'))
		else:
			self.BuildVariant.AddOperation(raptor.data.Append("UNIQUETARGETPATH","$(TARGET_var)_$(REQUESTEDTARGETEXT)",'/'))

		# Put the list of sourcefiles in with one Set operation - saves memory
		# and performance over using multiple Append operations.
		self.BuildVariant.AddOperation(raptor.data.Set("SOURCE",
						   " ".join(self.sources)))
		
		# Put the list of document files in with one Set operation - saves memory
		# and performance over using multiple Append operations.
		self.BuildVariant.AddOperation(raptor.data.Set("DOCUMENT",
						   " ".join(self.documents)))
	...
	def resolveDefFile(self, aTARGET, aBuildPlatform):
		...
		if platform in PlatformDefaultDefFileDir:
			...
			if secondaryDefaultDefFileDir:
				primaryFileCheck = raptor.utilities.resolveSymbianPath(self.__defFileRoot, resolvedDefFile, 'DEFFILE', "", str(aBuildPlatform['EPOCROOT']))
			...
			resolvedDefFile = raptor.utilities.resolveSymbianPath(self.__defFileRoot, resolvedDefFile, 'DEFFILE', "", str(aBuildPlatform['EPOCROOT']))
...
def CheckedGet(self, key, default = None):
	...
	raptor.meta for the use of everything inside raptor.meta.
...
raptor.data.Evaluator.CheckedGet = CheckedGet 
...
class MetaReader(object):
	...
	def __init__(self, aRaptor, configsToBuild):
		...
		evaluator = self.__Raptor.GetEvaluator(None, raptor.data.BuildUnit(metadata.name, [metadata]) )
	...
	def ReadBldInfFiles(self, aComponentList, doexport, dobuild = True):
		...
		for i,ep in enumerate(self.ExportPlatforms):
			filter = raptor.data.Filter(name = "export_" + str(i))
		...
		for i,bp in enumerate(self.BuildPlatforms):
			filter = raptor.data.Filter(name = "build_" + str(i))
			...
			# platform-wide data
			platformVar = raptor.data.Variant()
			platformVar.AddOperation(raptor.data.Set("PRODUCT_INCLUDE",
													 str(bp['VARIANT_HRH'])))
	...
	def AddComponentNodes(self, component, exportNodes, platformNodes):	
		...
		for i,ep in enumerate(self.ExportPlatforms):
			specNode = raptor.data.Specification(name = specName)

			# keep the BldInfFile object for later
			specNode.component = component

			# add some basic data in a component-wide variant
			var = raptor.data.Variant(name='component-wide')
			var.AddOperation(raptor.data.Set("COMPONENT_META", str(component.bldinf_filename)))
			var.AddOperation(raptor.data.Set("COMPONENT_NAME", component.name))
			var.AddOperation(raptor.data.Set("COMPONENT_LAYER", component.layername))
		...
		for i,bp in enumerate(self.BuildPlatforms):
			...
			if plat in platforms:
				...
				# add some basic data in a component-wide variant
				var = raptor.data.Variant(name='component-wide-settings-' + plat)
				var.AddOperation(raptor.data.Set("COMPONENT_META",str(component.bldinf_filename)))
				var.AddOperation(raptor.data.Set("COMPONENT_NAME", component.name))
				var.AddOperation(raptor.data.Set("COMPONENT_LAYER", component.layername))
				var.AddOperation(raptor.data.Set("MODULE_DIR", module_dir))
				var.AddOperation(raptor.data.Set("MODULE_NAME", module_name))
				var.AddOperation(raptor.data.Append("OUTPUTPATHOFFSET", outputDir, '/'))
				var.AddOperation(raptor.data.Append("OUTPUTPATH", outputDir, '/'))
				var.AddOperation(raptor.data.Append("BLDINF_OUTPUTPATH",outputDir, '/'))

				var.AddOperation(raptor.data.Set("TEST_OPTION", component.bldinf.getRomTestType(bp)))
				specNode.AddVariant(var)
	...
	def UnzipExport(self, _source, _destination, _sbs_build_dir, bldinf_filename):
		...
		sanitisedSource = raptor.utilities.sanitise(source)
		sanitisedDestination = raptor.utilities.sanitise(destination)
	...
	def ExtendedExport(self, from_dir, to_dir, bld_inf, args={}):
		...
		if recursive:
			...
			for root, dirs, files in os.walk(from_dir_str):
				found_files.extend([generic_path.Path(root, file) for file in fnmatch.filter(files, pattern_match)])
		else:
			...
			found_items = [from_dir.Append(item) for item in fnmatch.filter(os.listdir(from_dir_str), pattern_match)]
			found_files = [item for item in found_items if item.isFile()]
	...
	def ProcessTEMs(self, componentNode, buildPlatform):
		...
		for i,extension in enumerate(extensions):
			...
			extensionSpec = raptor.data.Specification("extension" + str(i))
			...
			var.AddOperation(raptor.data.Set("EPOCBLD", "$(OUTPUTPATH)"))
			var.AddOperation(raptor.data.Set("PLATFORM", buildPlatform["PLATFORM"]))
			var.AddOperation(raptor.data.Set("PLATFORM_PATH", buildPlatform["PLATFORM"].lower()))
			var.AddOperation(raptor.data.Set("CFG", "$(VARIANTTYPE)"))
			var.AddOperation(raptor.data.Set("CFG_PATH", "$(VARIANTTYPE)"))
			var.AddOperation(raptor.data.Set("GENERATEDCPP", "$(OUTPUTPATH)"))
			var.AddOperation(raptor.data.Set("TEMPLATE_EXTENSION_MAKEFILE", extension.getMakefile()))
			var.AddOperation(raptor.data.Set("TEMCOUNT", str(i)))
			...
			for standardVariable in standardVariables.keys():
				...
				var.AddOperation(raptor.data.Set(standardVariable, value))
			...
			var.AddOperation(raptor.data.Set("O._MEMBERS", ""))
			...
			for option in options:
				...
				if customInterface:
					var.AddOperation(raptor.data.Set(option, value))
				else:
					var.AddOperation(raptor.data.Append("O._MEMBERS", option))
					var.AddOperation(raptor.data.Set("O." + option, value))
	...
	def ProcessMMPs(self, componentNode, parser, buildPlatform):
		...
		for mmpFileEntry in MMPList['mmpFileList']:
			...
			mmpSpec = raptor.data.Specification(generic_path.Path(getSpecName(mmpFilename)))
			...
			if buildPlatform["TESTCODE"] and (mmpFileEntry.testoption in ["manual", "auto"]):
				...
				var.AddOperation(raptor.data.Set("TESTPATH",
			...
			var.AddOperation(raptor.data.Append("OUTPUTPATH","$(UNIQUETARGETPATH)",'/'))

			# If the bld.inf entry for this MMP had the BUILD_AS_ARM option then
			# tell the FLM.
			if mmpFileEntry.armoption:
				var.AddOperation(raptor.data.Set("ALWAYS_BUILD_AS_ARM","1"))
			...
			for export in componentNode.component.bldinf.getExports(buildPlatform):
				...
				if re.search('\.h',exportfile,re.IGNORECASE):
					var.AddOperation(raptor.data.Append("EXPORTHEADERS", str(exportfile)))
			...
			if not buildPlatform["ISFEATUREVARIANT"]:
				...
				for i,rvar in enumerate(backend.ResourceVariants):
					resourceSpec = raptor.data.Specification('resource' + str(i))
				...
				for i,stvar in enumerate(backend.StringTableVariants):
					stringTableSpec = raptor.data.Specification('stringtable' + str(i))
				...
				for i,bvar in enumerate(backend.BitmapVariants):
					bitmapSpec = raptor.data.Specification('bitmap' + str(i))
		...
		if not partialBuild:
			var = raptor.data.Variant()
			var.AddOperation(raptor.data.Set("TESTBATCHFILES","1"))
		...
		for g in MMPList['gnuList']:
			...
			var = raptor.data.Variant()
			gnuSpec = raptor.data.Specification("gnumakefile " + str(g.getMakefileName()))
			interface = buildPlatform["ext_makefile"]
			gnuSpec.SetInterface(interface)
			gnumakefilePath = raptor.utilities.resolveSymbianPath(str(bldInfFile), g.getMakefileName())
			var.AddOperation(raptor.data.Set("EPOCBLD", "$(OUTPUTPATH)"))
			var.AddOperation(raptor.data.Set("PLATFORM", buildPlatform["PLATFORM"]))
			var.AddOperation(raptor.data.Set("EXTMAKEFILENAME", g.getMakefileName()))
			var.AddOperation(raptor.data.Set("DIRECTORY",g.getMakeDirectory()))
			var.AddOperation(raptor.data.Set("CFG","$(VARIANTTYPE)"))
		...
		for m in MMPList['makefileList']:
			...
			var = raptor.data.Variant()
			gnuSpec = raptor.data.Specification("makefile " + str(m.getMakefileName()))
			interface = buildPlatform["ext_makefile"]
			gnuSpec.SetInterface(interface)
			gnumakefilePath = raptor.utilities.resolveSymbianPath(str(bldInfFile), m.getMakefileName())
			var.AddOperation(raptor.data.Set("EPOCBLD", "$(OUTPUTPATH)"))
			var.AddOperation(raptor.data.Set("PLATFORM", buildPlatform["PLATFORM"]))
			var.AddOperation(raptor.data.Set("EXTMAKEFILENAME", m.getMakefileName()))
			var.AddOperation(raptor.data.Set("DIRECTORY",m.getMakeDirectory()))
			var.AddOperation(raptor.data.Set("CFG","$(VARIANTTYPE)"))
			var.AddOperation(raptor.data.Set("USENMAKE","1"))
	...
	def unzippedPathFragment(self, sanitisedPath):
		fragment = hashlib.md5(sanitisedPath.encode('utf-8')).hexdigest()[:16]
		return fragment

...
class BootstrapFilter(filter_interface.Filter):
	...
	def write(self, text):
		...
		if text.startswith("<error>"):
			...
			self.out.write(str(raptor.build.name) + ": error: {0}\n".format(text[(start + 1):end]))
		elif text.startswith("<warning"):
			start = text.find(">")
			end = text.rfind("<")
			self.out.write(str(raptor.build.name) + ": warning: {0}\n".format(text[(start + 1):end]))
	...
	def open(self, raptor_instance, filternames, pbox):
		...
		for p in possiblefilters:
			...
			if name in filterdict:
				raise ValueError("filters found in SBS_HOME/raptor/plugins which have duplicate name: {0} ".format(p.__name__))
			else:
				filterdict[name] = p
		...
		if self.filters == []:
			...
			for filter in self.filters:
				...
				if ok:
					...
					sys.stderr.write(str(raptor.build.name) + \

...
'''
Filter classes for generating summary pages.

A base class SAX parser classifies text as Error, Warning etc. and calls
overridable reporting methods which generate the appropriate summary output.

Summary classes are CSV and HTML.
'''
...
class LogMessageClassifier(filter_interface.FilterSAX):
...
class HTML(LogMessageClassifier):
...
class CSV(LogMessageClassifier):

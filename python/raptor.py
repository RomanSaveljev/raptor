...
class ModelNode(object):
	...
	def check_uptodate(self, build):
		...
		build.Debug("check_uptodate: {0} ".format(makefile))
		try:
			makefilestat = os.stat(makefile)
			makefile_mtime = makefilestat[stat.ST_MTIME]
		except OSError, e:
			build.Info("incremental makefile generation: metadata is not uptodate: {0} doesn't exist".format(e.filename))
			return False

		metadepsfilename = makefile + ".metadeps"
		try:
			with open(metadepsfilename,"r+") as mdf:
				for l in mdf:
					toks=l.strip("\n\r ").split(": ")
					build.Debug("metadata deps: {0} {1}".format(toks[0],toks[1]))
					if toks[0] == "dep":
						depfile = toks[1].strip(" ")
						depstat = os.stat(depfile)
						deptime = depstat[stat.ST_MTIME]

						build.Debug("deptime: {0} makefiletime: {1}".format(deptime,makefile_mtime))
						if deptime > makefile_mtime:
							build.Debug("OUTOFDATE: metadata deps: {0} {1}".format(toks[0],toks[1]))
							return False
					elif toks[0] == "gnumakedeps":
						build.Debug("metadata gnumakedeps: {0} {1}".format(toks[0],toks[1]))
						gnudepfile = toks[1].strip(" ")
						with open(gnudepfile,"r") as gf:
							for depl in gf:
								gtoks = l.strip("\n\r \t\\").split(": ")
									
								build.Debug("gnumakedep: {0} : {1}".format(gtoks[0],gtoks[1]))
								if len(gtoks)>1:
									deplist = gtoks[1].split(" ")
								else:
									deplist = [gtoks[0].strip(" ")]
			...
			if build.incremental_parsing:
				build.Info("incremental makefile generation: metadata is not uptodate: {0} doesn't exist or has been altered".format(e.filename))
			return False

		if build.incremental_parsing:
			classname = str(self.__class__).strip("<>")
			classname = classname[classname.find('.')+1:-1]
			build.Info("incremental makefile generation: the makefile for the {0} named '{1}' is uptodate".format(classname,self.id))
		return True
	...
	def realise(self, build):
		...
		if build.incremental_parsing and uptodate:
			...
			m = build.maker.incremental_makefileset(generic_path.Path(thisnode_makefile))
		else:
			if build.incremental_parsing:
	...
	def Build(self):
		...
		try:
			...
			if self.doParallelParsing:
				...
				elements = ""
				for l in layers:
					elements += " ".join([c.id for c in l.children])

				environment = ";".join(["{0}={1}".format(k,os.environ[k]) for k in BuildRecord.sensed_environment_variables])


				# work out what makefiles we're going to use and use them.
				if self.incremental_parsing:
					self.build_record = BuildRecord.from_old(nodes = layers, adir = tm_dir, commandline=" ".join(self.args), environment = environment, elements = elements, topmakefile = str(makefile), configlist = configlist)
				...
				self.build_record.tofile(self.build_record_filename)

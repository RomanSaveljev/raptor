			...
			with open(metadepsfilename,"r") as mdf:
				for l in mdf:
					if l.startswith(ModelNode.dep_prefix):
						...
						build.Debug("metadata deps: {0}".format(depfile))
						...
						if deptime > makefile_mtime:
							build.Debug("OUTOFDATE: metadata deps: {0}".format(depfile))
							return False

					if l.startswith(ModelNode.include_prefix):
						gnudepfile = l[len(ModelNode.include_prefix):].strip("\r\n ")
						build.Debug("metadata gnumakedeps: {0}".format(gnudepfile))
						...
						with open(gnudepfile,"r") as gf:
							for dl in gf:
								...
								build.Debug("gnu make dep files: {0} ".format(str(deplist)))
	...
	def Build(self):
		...
		try:
			...
			if self.doParallelParsing:
				...
				self.build_record.to_file(self.build_record_filename)

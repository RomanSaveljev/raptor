...
if res:
	...
	print("Found object file {0}".format(objectpath))
	...
	if not os.path.isdir(objectdirectory):
		try:
			...
			print("Not making directory {0}".format(objectdirectory))
	...
	try:
		...
		print("Failed to created object file %s; error was: %s".format(objectfile, str(error)))
else:
	print("Failed to determine object filename. Commandline used was: {0}".format(" ".join(sys.argv[1:])))

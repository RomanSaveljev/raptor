...
# generated by the linker, then (a) copy the one just generated by the linker to
# a known location so it can be used in the compile stage of later builds and (b)
# delete the current set of object files that were just linked in order to enforce
# a re-compile in later builds.
# Note: as linker feedback files contain time-stamps, these are ignored in the diff.
#
define e32linkerfeedback_arm
	if [ ! -e $(FEEDBACKFILENAME) ] || [ -n "`$(GNUDIFF) -I ".*Last Updated:.*" --brief $(FEEDBACKFILENAME) $(FEEDBACKFILENAME)_temp`" ]; then \
	$(GNUCP) $(FEEDBACKFILENAME)_temp $(FEEDBACKFILENAME); \
	for OFILE in `$(GNUCAT) $(VIAFILE)` ; do $(GNURM) -rf $$$$OFILE; done; \
	fi;
endef
